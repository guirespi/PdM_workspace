...
bool_t delayRead(delay_t * delay)
{
	...
	else
	{
		...
		if(rt == true) delay->running = false;
	}
	return rt;
}
...
void delayWrite(delay_t * delay, tick_t duration)
{
	...
	if(delay->running == true) delay->running = false;
	delay->startTime = 0;
	delay->duration = duration;
}
...
void led_pwm_set_cfg(led_pwm_t * led, led_pwm_cfg_t * led_pwm_cfg)
{
	if(!led) return;
	if(!led->init) return;
	if(!led_pwm_cfg) return;
	if(led_pwm_cfg->duty_cycle > 100) return;
...
{
	if(!led) return;
	if(!led->init || !led->cfg) return;

	const uint32_t time_on = (uint32_t)(led->cycle*((float)led->duty_cycle/100));
